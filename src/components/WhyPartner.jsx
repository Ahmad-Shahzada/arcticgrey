...
// Use this to center the whole section or wrap it
import centralGraphic from '../../public/images/back-whiy-choose-mobile.webp';

const WhyPartner = () => {
    const [activeIndex, setActiveIndex] = useState(0);

    const items = [
        { title: 'Innovation that evolves', description: 'Pioneering AI solutions to keep your store at the forefront of eCommerce.' },
        { title: 'Solutions that scale', description: 'Flexible infrastructure built to grow alongside your business demands.' },
        { title: 'Data driven action', description: 'Turning complex analytics into actionable insights for better decision making.' },
        { title: 'Evolves with AI', description: 'Continuous learning systems that adapt to market trends in real-time.' },
    ];

    const brands = [
        'SMARTRI', 'TRIPLE WHALE', 'AUTOMMIC', 'SHOP LIFT',
        'GROOTAPPS', 'NETSUITE', 'MATRIXIFY', 'OKENDO'
    ];
    ...
    return (
        <>
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-start z-10 overflow-hidden pointer-events-none">
                                <div className="flex flex-col items-center w-full animate-scroll-up-continuous">
                                    {scrollingBrands.map((brand, index) => (
                                        <div key={index} className="flex justify-center items-center py-6 md:py-8 w-full">
                                ...
                                <div className="absolute inset-x-0 top-0 h-16 md:h-24 bg-gradient-to-b from-[#1a1a1a]/40 to-transparent"></div>
                                <div className="absolute inset-x-0 bottom-0 h-16 md:h-24 bg-gradient-to-t from-[#1a1a1a]/40 to-transparent"></div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-4 md:gap-6 relative z-10 w-full max-w-full lg:max-w-[480px] mx-auto">
                            {items.map((item, index) => {
                                ...
                                return (
                                    <div
                                        key={index}
                                        className="flex flex-col border-b border-[#E1E1E1] pb-4 md:pb-6 last:border-none cursor-pointer group"
                                        onClick={() => setActiveIndex(index)}
                                    >
                                        <div className="flex items-center justify-between gap-4 w-full">
                                            <h3 className={`transition-colors duration-300 text-[16px] md:text-[19px] font-medium ${isOpen ? 'text-black' : 'text-gray-700'}`}>
                                                {item.title}
                                            </h3>
                                            <div className="ml-auto">
                                                <Asterisk
                                                    size={24}
                                                    strokeWidth={2}
                                                    className={`transition-all duration-500 ease-in-out ${isOpen ? 'text-black rotate-90 scale-110' : 'text-gray-400'}`}
                                                />
                                            </div>
                                        </div>
            ...
            {/* LOGO SECTION - Responsive wrapping */}
            <section className="w-full bg-[#fbf7ed] py-12 px-6">
