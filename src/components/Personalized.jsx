                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 w-full px-6 py-12 md:px-16 lg:py-20 items-center">
                    ...
                    <div className="flex flex-col space-y-8 md:space-y-10 z-10 order-1 lg:order-2 lg:ml-10 text-left items-center lg:items-start">
                        ...
                        <div className="space-y-4 md:space-y-6">
                            <h1 className="text-2xl md:text-4xl lg:text max-w-md leading-[1.15] tracking-tight font-semibold">
                                Fully Automated Shopify Store With 
                                <span className='inline pl-3' style={{ backgroundImage: goldGradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                                    AI Agents
                                </span>
                            </h1>
                            <p className="text-zinc-400 text-sm md:text-base lg:text-lg max-w-lg leading-relaxed font-light mx-auto lg:mx-0">
                                Create seamless, personalized shopping experiences with Arctic Grey’s end-to-end Shopify Product Customizers that drive engagement and operational efficiency.
                            </p>
                        </div>
                        ...
                        <div className="w-full max-w-lg text-left">
                            {features.map((feature, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        <span className={`text-base md:text-lg font-medium transition-colors duration-300 ${isHighlighted ? 'text-white' : 'text-white'}`}>
                                            {feature.title}
                                        </span>
                                        <div className={`transition-colors duration-300 ${isHighlighted ? 'text-white' : 'text-white'}`}>
                                            {feature.icon}
                                        </div>
                                    </div>
                                );
                        ...
                        <div className="pt-4 md:pt-8 w-full sm:w-auto">
                            <div
                                ...
                                className="inline-block p-[1px] md:p-[2px] rounded-full group cursor-pointer w-full sm:w-auto"
                            >
                                <button className="flex items-center justify-center gap-2.5 hover:bg-white hover:text-black px-6 md:px-8 py-3.5 md:py-4 bg-black text-white text-sm md:text-lg font-semibold rounded-full w-full transition-all duration-300">
                                    <span>
                                        Build My Custom AI Agent Team
                                        <span className="inline-block group-hover:translate-x-2 transition-transform duration-300 italic pl-2 md:pl-3">→</span>
                                    </span>
                                </button>
                            </div>
                        </div>
