...
export default function HeroSection() {
    ...
    // --- New Attractive Green Gradient ---
    const greenGradient = "linear-gradient(135deg, #22c55e 0%, #10b981 50%, #059669 100%)";
    const darkGreenBorder = "rgba(16, 185, 129, 0.3)";

    const tickerItems = [
        "Unified Shopify Development", "1,000+ Stores Supported", "$5B+ In Merchant Revenue",
        "Proven Results & Backed By Data", "D2C & B2B Shopify Expertise", "Award-Winning Developers"
    ];
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 mx-auto px-4 sm:px-6 lg:px-10 pb-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12 md:mx-10 items-center mb-16 md:mb-20 pt-10">
                    ...
                    <div className="space-y-6 md:space-y-8 lg:mt-10 text-center lg:text-left">
                        <h1 className="text-4xl sm:text-[54px] font-bold leading-tight text-white">
                            ...
                            <span style={{ backgroundImage: greenGradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                                Shopify Support
                            </span>
                        </h1>
                        <p className="text-gray-300 text-lg lg:text-[22px] max-w-lg mx-auto lg:mx-0">
                            <span className="text-emerald-400 font-medium">Award-Winning </span>Shopify Developers.
                            Fixes, Upgrades, New Builds & <span className="text-emerald-400">Migrations. Work starts in hours.</span>
                        </p>
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-6 md:gap-8">
                            ...
                            <button className="relative group w-full sm:w-auto px-10 py-4 rounded-full font-bold overflow-hidden text-white transition-all">
                                ...
                                <div className="absolute inset-0 transition-transform duration-300 group-hover:scale-105" style={{ background: greenGradient }}></div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                        {[
                            ...
                            <div
                                key={i}
                                className="group bg-black backdrop-blur-sm cursor-pointer p-6 sm:p-8 rounded-[30px] border-4 border-white/10 flex flex-col items-start text-left hover:border-emerald-500/50 transition-all duration-500"
                            >
                                {/* --- ICON (Changes to Green) --- */}
                                <div className="text-white group-hover:text-emerald-400 mb-6 transition-colors duration-300">
                                    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                                        <path d={card.icon} strokeLinecap="round" strokeLinejoin="round" />
                                        {i === 0 && <path d="M17 6h6v6" strokeLinecap="round" strokeLinejoin="round" />}
                                    </svg>
                                </div>
                                ...
                                {/* --- BUTTON --- */}
                                <button 
                                    className="w-full py-3 border border-emerald-500/30 bg-emerald-500/10 rounded-xl text-emerald-400 font-bold group-hover:bg-emerald-500 group-hover:text-black transition-all duration-300 mb-6"
                                >
                                    {card.btnText} <span className="ml-1">→</span>
                                </button>

                                <div className="w-full text-center">
                                    <a href="#" className="text-emerald-500/80 text-xs font-bold underline underline-offset-4 hover:text-emerald-400">{card.footerLink} →</a>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                {/* --- TICKER --- */}
                <div className="relative overflow-hidden w-full py-5 border-y border-white/5 mb-10 md:mb-12 md:mx-10 bg-emerald-950/20">
                    <div className="animate-scroll whitespace-nowrap">
                        {[...tickerItems, ...tickerItems].map((item, i) => (
                            <div key={i} className="inline-flex items-center mx-4 sm:mx-8">
                                <span className="w-2 h-2 bg-emerald-500 rounded-full mr-4 shadow-[0_0_8px_#10b981]"></span>
                                <span className="text-gray-300 text-xs sm:text-sm font-medium tracking-wide uppercase">{item}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-16 md:mx-10">
                    {[
                        ...
                        <div key={i} className="bg-white/5 backdrop-blur-md p-5 sm:p-6 rounded-[14px] border border-white/10 flex justify-between items-center group hover:border-emerald-500/30 transition-all">
                            <div className="flex-1">
                                <p className="text-[10px] sm:text-[11px] text-gray-400 font-bold uppercase tracking-tighter mb-2">{stat.label}</p>
                                <div style={{ backgroundImage: greenGradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }} className="text-sm sm:text-base font-black flex items-center">
