...
export default function PlatinumPartnerSection() {
    ...
    return (
                    ...
                    <div className="flex flex-col gap-16">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[1fr,1.3fr] gap-x-12 xl:gap-x-16 gap-y-10 pt-5 lg:pt-0 items-start relative">
                            ...
                            <div className="lg:w-[700px] flex flex-col gap-6 p-5 lg:p-20 relative z-20">
                                <h1 className="text-2xl lg:text-4xl leading-[1.15] tracking-tight">
                                    Arctic Grey is the #1 Highest Ranked <span className="text-[#BF905E]">Platinum</span> Partner on Shopify Worldwide
                                </h1>
                            </div>
                        ...
                        <div className="cursor-pointer grid grid-cols-1 md:grid-cols-3 px-5 lg:px-10 pb-20 gap-10 lg:gap-14 w-full">
                            {team.map((member, index) => (
                                ...
                                <div
                                    key={index}
                                    className={`relative group flex flex-col items-center ${member.bg} px-10 rounded-[24px] shadow-2xl transition-all duration-300 ease-in-out overflow-hidden z-10`}
                                >

                                    <div
                                        className="absolute top-[-5px] left-[-5px] w-[150%] h-[150%] z-0 rounded-[40px] pointer-events-none transition-opacity duration-300 opacity-0 group-hover:opacity-100"
                                        style={{
                                            // Use the precise gold gradient hex from the design if not defined in config
                                            backgroundImage: 'linear-gradient(135deg, #BF905E 0%, #F5E6B3 40%, #BF905E 80%)',

                                            maskImage: `
                                                linear-gradient(to right, black 2px, transparent 2px),
                                                linear-gradient(to bottom, black 2px, transparent 2px),
                                                radial-gradient(circle at top left, transparent 2px, transparent 2px, black 3px)
                                            `,
                                            WebkitMaskImage: `
                                                linear-gradient(to right, black 2px, transparent 2px),
                                                linear-gradient(to bottom, black 2px, transparent 2px),
                                    ...
                                    <div className="w-full text-center flex flex-col items-center gap-2 my-10 relative z-10">
                                        <h3 className="text-[20px] md:text-[22px] font-bold text-black tracking-tight">{member.name}</h3>
                                        <p className="text-[16px] font-normal text-gray-700">{member.role}</p>
                                    </div>
      ...
      `}</style>
            ...
            <section className="relative w-full min-h-[400px] md:min-h-[500px] flex flex-col items-center justify-center bg-[#fcfaf6] overflow-hidden py-16 md:py-24 px-5 md:px-12">
