...
export default function Ai_apps() {
  const cards = [
    {
      ...
      description:
        'Our AI algorithm delivers dynamic product feeds, boosting conversions and average order value, giving your store a competitive edge in personalization.',
    },
    {
      image: '/images/ai-powered-2.webp',
      title: 'AI Speed Optimization',
      description:
        'AI Speed enhances Core Web Vitals, potentially increasing conversions by 7% per second saved, ensuring your store outperforms competitors in user experience.',
    },
  ];
  ...
  return (
    <div className="bg-light-bg pt-6 md:pt-10 px-4 md:px-5">
        ...
        <div className="w-full flex flex-col items-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10 lg:gap-16 w-full mb-16 md:mb-20">
            {cards.map((card, index) => (
              <div
                key={index}
                className="flex flex-col items-start bg-[#0D0D0D] md:p-10 rounded-[20px] md:rounded-[30px] "
              >
                ...
                <div className="w-full flex flex-wrap items-center justify-start gap-4 md:gap-5 mt-auto">
                  <a
                    href="#"
                    className="border-2 border-primary-soft hover:border-white py-2 md:py-3 px-4 md:px-5 rounded-full transition-all shrink-0"
                  >
                    <img
                  ...
                  <a
                    href="#"
                    className="flex items-center gap-1.5 text-base md:text-[18px] font-medium text-white transition-colors"
                  >
                    Get Started for
                    <span className="text-primary-accent font-semibold underline">
                      Free!
                    </span>
                  </a>
                </div>
            ...
            <div className="w-full max-w-[1200px] overflow-hidden relative">
              ...
              <div className="flex items-center gap-12 md:gap-20 w-max animate-scroll-left">
                {logosToScroll.map((partner, idx) => (
                  <div
                    key={idx}
                    className="flex-shrink-0 grayscale opacity-80 hover:opacity-100 transition-opacity"
                  >
                    <img
                      src={partner.logoSrc}
                      alt={partner.name}
