...
export default function CompetitiveEdge() {
  ...
  return (
    <>
      {/* DESKTOP */}
      <section className="hidden lg:block px-5 pb-20 md:pb-32 overflow-hidden">
        <style>{`
        ...
        <div className="pb-4 grid grid-cols-1 lg:grid-cols-[1.1fr,0.9fr] bg-white rounded-xl gap-8 items-start">
          ...
          <div className="flex flex-col h-full w-full justify-end order-2 py-8 pl-20 pr-10 lg:order-1 lg:mt-0">
            <div className="mb-12 absolute top-[1620px]">
              {/* Optional small label using primary accent */}
              <span className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-primary-accent mb-4">
                Competitive Edge
              ...
              <p className="text-[#666] text-[17px] max-w-[820px]">
                Arctic Grey offers specialized optimizations like UX/UI audits, speed enhancements,
                mobile-first themes, and custom apps to ensure real-time synchronization and
                personalized experiences, while also handling complex setups such as loyalty
                programs, product customizers, and in-store kiosks that bridge online and offline
                retail.
              </p>
            </div>
              ...
              {features.map((item, index) => (
                ...
                >
                  <div className="flex justify-between items-center py-7 group">
                    <span
                      className={`text-md md:text-xl font-medium transition-all duration-300 ${
                        activeTab === index
                          ? 'text-primary-accent'
                          : 'text-black group-hover:text-primary-accent'
                      }`}
                    >
                      {item.title}
                    </span>
                    <div
                      className={`transition-transform duration-300 ${
                        activeTab === index
                          ? 'rotate-90 scale-110 text-primary-accent'
                          : 'text-black/60 group-hover:text-primary-accent'
                      }`}
                    >
                      {item.icon}
                    </div>
                  ...
                  <div
                    className={`overflow-hidden transition-all duration-500 ease-in-out ${
                      activeTab === index ? 'max-h-32 pb-5 opacity-100' : 'max-h-0 opacity-0'
                    }`}
                  >
          ...
          <div className="relative w-full flex flex-col items-center order-1 lg:order-2">
            <div className="relative w-[740px] rounded-[10px] shadow-sm">
              ...
              />
              <div className="absolute inset-0 lg:bg-white opacity-30 mix-blend-multiply" />
            </div>
            ...
            <div className="relative z-10 w-full lg:bg-white rounded-[32px] p-10 md:p-8 shadow-[0_30px_80px_rgba(0,0,0,0.12)] -mt-32 md:-mt-48 lg:-mt-80 lg:m-5 max-w-[600px]">
              <div className="text-center">
                <h3 className="text-7xl font-bold tracking-tighter mb-4 bg-primary-gradient bg-clip-text text-transparent">
                  1000+
                </h3>
                ...
                <div className="relative w-full overflow-hidden mask-fade">
                  <div className="animate-company-ticker">
                    {[...companies, ...companies].map((company, index) => (
                      <span
                        key={index}
                        className={`${company.weight} text-lg md:text-xl tracking-tighter text-black/30 uppercase italic whitespace-nowrap`}
                      >
                        {company.name}
                      </span>
                    ))}
        ...
        <div className="max-w-7xl mx-auto bg-white rounded-[2rem] overflow-hidden shadow-sm grid grid-cols-1 lg:grid-cols-2">
          <div className="p-6 sm:p-12 lg:p-20 order-2 lg:order-1 flex flex-col justify-center">
            <div className="mb-8 lg:mb-12">
              ...
              <p className="text-zinc-500 text-sm sm:text-base lg:text-lg max-w-xl leading-relaxed">
                Arctic Grey offers specialized optimizations like UX/UI audits, speed enhancements,
                and mobile-first themes to ensure real-time synchronization and personalized
                experiences.
              </p>
            </div>
              ...
              {features.map((item, index) => (
                ...
                >
                  <div className="flex justify-between items-center py-5 sm:py-7 group">
                    <span
                      className={`text-base sm:text-lg lg:text-xl font-semibold transition-all duration-300 ${
                        activeTab === index ? 'text-primary-accent' : 'text-zinc-400 group-hover:text-primary-accent'
                      }`}
                    >
                      {item.title}
                    </span>
                    <div
                      className={`text-xl transition-transform duration-300 ${
                        activeTab === index
                          ? 'rotate-90 scale-125 text-primary-accent'
                          : 'grayscale opacity-50 group-hover:opacity-100 group-hover:text-primary-accent'
                      }`}
                    >
                      {item.icon}
                    </div>
                  ...
                  <div
                    className={`overflow-hidden transition-all duration-500 ease-in-out ${
                      activeTab === index ? 'max-h-40 pb-6 opacity-100' : 'max-h-0 opacity-0'
                    }`}
                  >
          ...
          <div className="relative w-full flex flex-col items-center order-1 lg:order-2 bg-[#F9F9F9] lg:bg-transparent">
            ...
            <div className="relative z-10 w-[90%] sm:w-[80%] lg:w-[85%] bg-white rounded-3xl p-8 lg:p-12 shadow-[0_20px_60px_rgba(0,0,0,0.1)] -mt-24 sm:-mt-32 lg:-mt-40 xl:-mt-52 mb-12 lg:mb-0">
              <div className="text-center">
                <h3 className="text-5xl sm:text-6xl lg:text-7xl font-black tracking-tighter mb-2 bg-primary-gradient bg-clip-text text-transparent">
                  1000+
                </h3>
